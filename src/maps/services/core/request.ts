...
import type { AxiosError, AxiosRequestConfig, AxiosResponse, AxiosInstance, AxiosResponseHeaders } from "axios";
...
export const ACCESS_TOKEN_KEY = "jadx_vis_web_access_token1";
export const REFRESH_TOKEN_KEY = "jadx_vis_web_refresh_token1";
...
export const sendRequest1 = async <T>(
...
export const sendRequest = async <T>(
  ...
  try {
    ...
    if (axiosError.response?.status === 401 && (axiosError.response.data as any)?.detail?.includes("token")) {
      ...
      try {
        ...
        if (refreshToken) {
          // const newTokens = await LoginService.getRefreshToken({
