...
const useCropDistributionLayer = ({ layerManager, map, ready, selectedLvl, setMenuPosition, setMenuChildren, cropData, hexData, opacity, areaData }: Params) => {
  ...
  useEffect(() => {
    ...
    cropData.features.forEach((feature: any) => {
      ...
      const background: CropColor | "white" = CROP_COLORS[CROPS[cropName]] || "white";
      const color = ["#e4fc6f", "#FFFF00", "#FF6347", "#98FB98", "#00FF00"].includes(background) ? "black" : "white";
          ...
          const children = (
            <div>
              ...
              {pummokData.map((item, index) => {
                ...
                return <div key={index}>{`${item.pummok}: ${ha.toFixed(2)} ha`}</div>;
              })}
            </div>
          );
          setMenuPosition({ x: Math.round(event.pixel[0]), y: Math.round(event.pixel[1]) });
          ...
          features.forEach((feature) => {
            if (featureAtPixel && feature.get("feature_type") != "crop") {
              if (feature == highlightedFeature) {
                ...
                const crops: CropLabel[] = cropMap[region]?.split(", ").map((c) => c.trim()) || [];

                // 작물별 div 생성
                const cropsHtml = crops
                  .map((crop) => {
                    const background = CROP_COLORS[CROPS[crop]] || "#000000";
                    const color = ["#e4fc6f", "#FFFF00", "#FF6347", "#98FB98", "#00FF00"].includes(background) ? "black" : "white";
                    return `  
