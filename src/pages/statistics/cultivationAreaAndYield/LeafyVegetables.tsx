...
const WheatAndBarleyMilledGrain = () => {
  ...
  const getSelectFields = (currentSelections: Record<string, string | string[]>): SelectField[] => {
    ...
    if (usage === "면적") {
      dynamicOptions = ["엽채류:면적", "노지배추:면적", "시설배추:면적", "시금치:면적", "상추:면적", "양배추:면적"];
      defaultSelected = ["노지배추:면적", "양배추:면적"];
    } else if (usage === "생산량") {
      dynamicOptions = ["엽채류:생산량", "노지배추:생산량", "시설배추:생산량", "시금치:생산량", "상추:생산량", "양배추:생산량"];
      defaultSelected = ["노지배추:생산량", "양배추:생산량"];
      ...
      defaultSelected = ["노지배추:10a당 생산량", "양배추:10a당 생산량"];
    }
  ...
  return (
    <StatisticsContainer title="채소류 - 엽채류">
