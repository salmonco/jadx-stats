...
const QUALIFIED_NAME = "whtbrl-mldgrn";
const UNIT = { 면적: "ha", 생산량: "톤", "10a당 생산량": "kg" };

// 식량작물 - 맥류(정곡)
const WheatAndBarleyMilledGrain = () => {
  ...
  const getSelectFields = (currentSelections: Record<string, string | string[]>): SelectField[] => {
    ...
    if (usage === "면적") {
      dynamicOptions = ["합계:면적", "겉보리:면적", "쌀보리:면적", "맥주보리:면적", "밀:면적"];
      defaultSelected = ["쌀보리:면적", "맥주보리:면적", "밀:면적"];
    } else if (usage === "생산량") {
      dynamicOptions = ["합계:생산량", "겉보리:생산량", "쌀보리:생산량", "맥주보리:생산량", "밀:생산량"];
      defaultSelected = ["쌀보리:생산량", "맥주보리:생산량", "밀:생산량"];
    } else if (usage === "10a당 생산량") {
      dynamicOptions = ["겉보리:10a당 생산량", "쌀보리:10a당 생산량", "맥주보리:10a당 생산량", "밀:10a당 생산량"];
      defaultSelected = ["쌀보리:10a당 생산량", "맥주보리:10a당 생산량", "밀:10a당 생산량"];
    }
  ...
  return (
    <StatisticsContainer title="식량작물 - 맥류(정곡)">
...
export default WheatAndBarleyMilledGrain;
