...
const QUALIFIED_NAME = "miscgrn";
const UNIT = { 면적: "ha", 생산량: "톤", "10a당 생산량": "kg" };

// 식량작물 - 잡곡
const MiscellaneousGrains = () => {
  ...
  const getSelectFields = (currentSelections: Record<string, string | string[]>): SelectField[] => {
    ...
    if (usage === "면적") {
      dynamicOptions = ["합계:면적", "옥수수:면적", "메밀:면적", "기타잡곡:면적"];
      defaultSelected = ["옥수수:면적", "메밀:면적", "기타잡곡:면적"];
    } else if (usage === "생산량") {
      dynamicOptions = ["합계:생산량", "옥수수:생산량", "메밀:생산량", "기타잡곡:생산량"];
      defaultSelected = ["옥수수:생산량", "메밀:생산량", "기타잡곡:생산량"];
    } else if (usage === "10a당 생산량") {
      dynamicOptions = ["옥수수:10a당 생산량", "메밀:10a당 생산량", "기타잡곡:10a당 생산량"];
      defaultSelected = ["옥수수:10a당 생산량", "메밀:10a당 생산량", "기타잡곡:10a당 생산량"];
    }
  ...
  return (
    <StatisticsContainer title="식량작물 - 잡곡">
...
export default MiscellaneousGrains;
