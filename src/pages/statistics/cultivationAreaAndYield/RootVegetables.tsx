...
const RootVegetables = () => {
  ...
  const getSelectFields = (currentSelections: Record<string, string | string[]>): SelectField[] => {
    ...
    if (usage === "면적") {
      dynamicOptions = ["근채류:면적", "무:면적", "노지겨울무:면적", "당근:면적"];
      defaultSelected = ["노지겨울무:면적", "당근:면적"];
    } else if (usage === "생산량") {
      dynamicOptions = ["근채류:생산량", "무:생산량", "노지겨울무:생산량", "당근:생산량"];
      defaultSelected = ["노지겨울무:생산량", "당근:생산량"];
    } else if (usage === "10a당 생산량") {
      dynamicOptions = ["노지무:10a당 생산량", "노지겨울무:10a당 생산량", "당근:10a당 생산량"];
      defaultSelected = ["노지겨울무:10a당 생산량", "당근:10a당 생산량"];
    }
  ...
  return (
    <StatisticsContainer title="채소류 - 근채류">
